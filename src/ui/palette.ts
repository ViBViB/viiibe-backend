        ...
        bar.onclick = async () => {
            ...
            try {
                ...
                if (navigator.clipboard && navigator.clipboard.writeText) {
                    ...
                    copySuccess = true;
                    console.log('✅ Copied using clipboard API:', hexValue);
                }
            } catch (err) {
                console.log('⚠️ Clipboard API failed, trying fallback:', err);
            }

            // Method 2: Fallback using textarea (more reliable in iframes)
            if (!copySuccess) {
                try {
                    ...
                    if (successful) {
                        copySuccess = true;
                        console.log('✅ Copied using execCommand:', hexValue);
                    } else {
            ...
            if (copySuccess) {
                ...
                console.error('❌ All copy methods failed for:', hexValue);
            }
        };
