export function generateTypographySystem() {
    ...
    container.innerHTML = `<div class="font-pair-card" style="margin-bottom:24px; display:grid; grid-template-columns:1fr 1fr 1.3fr; gap:16px; align-items:start; border-top:1px solid #E0E0E0; padding-top:16px;"><div class="font-preview-box"><span class="font-label">Heading</span><div class="font-name" style="font-family:'${pair.head.family}'; font-size:13px; font-weight:600; color:#000;">${pair.head.family}</div></div><div class="font-preview-box" style="border-left:1px solid #eee; padding-left:16px;"><span class="font-label">Body</span><div class="font-name" style="font-family:'${pair.body.family}'; font-size:13px; font-weight:600; color:#000;">${pair.body.family}</div></div><div class="font-preview-box" style="border-left:1px solid #eee; padding-left:16px;"><span class="font-label">Scale System</span><div style="font-size:13px; font-weight:600; color:#000;">${scale.name} (${scale.ratio})</div><div style="font-size:11px; color:#888; line-height:1.3; margin-top:2px;">${desc}</div></div></div><div class="scale-list">${listHtml}</div>`;
}
